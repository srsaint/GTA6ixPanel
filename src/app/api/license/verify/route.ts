...
export async function POST(request: Request) {
  ...
  if (!input.success) return apiError("bad_request", "License key and device ID are required.");
  ...
  if (recentFailures >= 20) return apiError("rate_limited", "Too many attempts. Try again later.", 429);
  ...
  if (!license) return finish("invalid_key", apiError("invalid_key", "License key not found.", 404));
  if (license.status === "BANNED") return finish("banned", apiError("banned", "License key was disabled.", 403));
  if (license.expiresAt.getTime() < Date.now()) return finish("expired", apiError("expired", "License key expired.", 403));
  ...
  if (existingDevice) {
    ...
    if (license.devices.length >= license.maxDevices) {
      return finish("device_limit", apiError("device_limit", "License key is already bound to another device.", 403));
    }
  ...
  return finish("ok", apiOk({ verifiedUntil, expiresAt: license.expiresAt.getTime(), licenseStatus: license.status }));
}
